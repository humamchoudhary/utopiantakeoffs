...
const components = {
  ...
  p: (props) => {
    ...
    if (children.length === 1 && React.isValidElement(children[0]) && children[0].type.name === 'img') {
  ...
  img: ({ src, alt, ...props }) => {
    ...
    try {
      ...
      if (src.startsWith('http')) {
        ...
        return <img src={src} alt={alt || "Image"} className="my-4 rounded-lg max-w-full h-auto" />;
      }
    } catch (error) {
