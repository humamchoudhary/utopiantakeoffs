...
const ReviewCard = ({ name, image, currentIdx, index }) => (
  ...
  >
    {index != 0 && name && (
      <div
        className={`bg-fg w-full flex flex-col justify-end rounded-[24px] px-6 py-8 ${currentIdx == index ? "h-full" : "h-full md:h-2/3"} duration-150 `}
      >
...
const ReviewSection = ({ a_reviews, w_reviews }) => {
  ...
  return (
    <div className="w-full px-4 py-8 md:py-32 md:px-14 md:max-w-screen-2xl flex flex-col items-center justify-center">
      ...
      <div className="flex flex-row items-center overflow-hidden w-full md:w-[1328px] relative md:px-4">
        {loaded && instanceRef.current && (
          <button
            ...
            className={`rounded-full p-2 md:p-3 z-50 ${
        ...
        <div className="overflow-hidden self-start w-full relative">
          ...
          {loaded &&
            instanceRef.current.track.details.maxIdx !== currentSlide && (
              <div className="z-10 right-0 top-0 absolute pointer-events-none h-full w-[20px] md:w-[450px] bg-gradient-to-r from-transparent to-bg" />
            )}
        </div>
        {loaded && instanceRef.current && (
          <button
