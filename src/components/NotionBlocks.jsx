...
export const NotionBlock = ({ block }) => {
  ...
  switch (block.type) {
    ...
    case 'heading_1':
      return <h1 className="text-3xl font-bold my-6 mt-24">{block.heading_1.text}</h1>;
    case 'heading_2':
      return <h2 className="text-2xl font-bold my-5 mt-16">{block.heading_2.text}</h2>;
    case 'heading_3':
      return <h3 className="text-xl font-bold my-4 mt-12">{block.heading_3.text}</h3>;
    case 'bulleted_list_item':
      return <li className="ml-6 list-disc my-2">{block.bulleted_list_item.text}</li>;
    case 'numbered_list_item':
      return <li className="ml-6 list-decimal my-2">{block.numbered_list_item.text}</li>;
    case 'to_do':
      return renderToDo(block);
    case 'toggle':
      return renderToggle(block);
    case 'child_page':
      return <div className="my-4 p-4 border border-gray-200 rounded-lg">{block.child_page.title}</div>;
    case 'image':
    ...
    default:
      return <div className="my-4 text-gray-500">Unsupported block type: {block.type}</div>;
  }
};
...
function renderToggle(block) {
  return (
    <details className="my-4">
      <summary className="cursor-pointer font-medium">{block.toggle.text}</summary>
  ...
  return (
    ...
    >
      ...
      {block.bookmark.caption && (
        <div className="text-gray-500 text-sm mt-2">{block.bookmark.caption}</div>
      )}
    </a>
  );
}
